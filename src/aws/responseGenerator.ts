 ...
 */
async function getEmailQuery(queryId: string): Promise<EmailQuery | null> {
    try {
        ...
        return (result.Item as EmailQuery) || null;
    } catch (error) {
...
async function updateConversationWithResponse(
    ...
    await docClient.send(
        new UpdateCommand({
            ...
            Key: { conversationId },
            UpdateExpression:
                'SET messages = list_append(messages, :message), lastUpdated = :lastUpdated, totalCost = totalCost + :cost, totalTokens = totalTokens + :tokens',
 ...
 */
function formatConversationHistory(conversation: Conversation): string {
...
async function generateLLMResponse(
    ...
    modelName: string,
    attachments: EmailAttachment[]
): Promise<{ content: string; cost: number; tokens: number }> {
    try {
 ...
 */
async function storeGeneratedResponse(response: GeneratedResponse): Promise<void> {
    await docClient.send(
        new PutCommand({
 ...
 */
async function getInfrastructureCost(): Promise<number> {
    try {
 ...
 */
async function getLicensingCost(): Promise<number> {
    try {
        const result = await ssmClient.send(
            new GetParameterCommand({
...
export async function generateResponse(
    ...
    try {
        ...
        if (conversation) {
            ...
            await updateConversationWithResponse(
        ...
        const messageId = await sendEmailResponse(
            query.userEmail,
            query.fromSender,
            responseSubject,
            responseBody
        ...
        // Update query status to sent (response delivered)
 ...
 */
export async function processResponseQueue(): Promise<void> {
    try {
...
export async function handler(event: any): Promise<any> {
    try {
        ...
        if (event.Records && event.Records[0].eventSource === 'aws:sqs') {
            ...
            // Handle test scenario where no queryId is provided
            if (!queryId && body.conversationId) {
                console.log('Test scenario detected - creating test email query');
                const testQueryId = generateUUID();
                
                // Create a test email query
                const testQuery: EmailQuery = {
                    queryId: testQueryId,
                    gmailId: 'test-gmail-id',
                    userEmail: body.userEmail || 'test@example.com',
                    fromSender: 'sender@example.com',
                    subject: 'Test Email Subject',
                    body: body.emailContent || 'This is a test email content for response generation.',
                    attachments: [],
                ...
                console.log(`Created test query with ID: ${testQueryId}`);
                
                const result = await generateResponse(testQueryId);
                return {
            ...
            if (!queryId) {
                return {
                    statusCode: 400,
                    body: JSON.stringify({ error: 'No queryId provided in message' }),
                };
            }
