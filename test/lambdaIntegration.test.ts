...
import { getLatestChatterboxGmail, getGmailById, getEmailSubject, getEmailSender } from '../src/mail/getGmail';
...
async function readSendCount(): Promise<number> {
    try {
        ...
        if (await fs.access(sendCountPath).then(() => true).catch(() => false)) {
...
async function getMostRecentChatterboxIdLocal(authClient: any): Promise<string | null> {
    try {
        ...
        const latestEmail = await getLatestChatterboxGmail(config.app.defaultGetGmailUser, authClient);
        ...
        const fullEmail = await getGmailById(latestEmail.id, config.app.defaultGetGmailUser, authClient);
