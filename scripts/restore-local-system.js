    ...
    try {
        ...
        const backupDirs = items
            .filter(
                (item) =>
                    item.isDirectory() &&
                    (item.name.startsWith('backup-') || item.name.startsWith('clean-'))
            )
            .map((item) => ({
                name: item.name,
...
function restoreFile(backupFile, targetDir = '.') {
    try {
        ...
        console.error(
            chalk.red(`   ❌ Failed to restore ${backupFile.originalName}: ${error.message}`)
        );
        return false;
    }
}
...
async function promptUser(question, defaultValue = false) {
    ...
    const answer = await new Promise((resolve) => {
        rl.question(
            chalk.yellow(`${question} (${defaultValue ? 'Y' : 'n'}/${defaultValue ? 'y' : 'N'}): `),
            (ans) => {
                rl.close();
...
async function selectBackupDirectory() {
    ...
    const choices = backupOptions.map((option) => {
        const notes =
            option.metadata.notes === 'No notes provided' ||
            option.metadata.notes === 'No metadata available'
                ? 'No more details'
...
function showUsage() {
    ...
    console.log(
        chalk.gray(
            '  backup-dir    Directory containing backup files (default: interactive selection)'
        )
    );
    console.log('');
    ...
    console.log(
        chalk.gray('  npm run restore:local                    # Interactive backup selection')
    );
    console.log(
        chalk.gray(
            '  npm run restore:local ./backups/custom   # Restore from specific backup directory'
        )
    );
    console.log(
        chalk.gray(
    ...
    console.log(
        chalk.gray(
            '  npm run restore:local --target-dir ./new # Restore to different target directory'
        )
    );
    console.log('');
...
async function main() {
    ...
    if (backupFiles.length === 0) {
        ...
        console.log(
            chalk.gray('Make sure you have run the clean script with backup enabled first.')
        );
        process.exit(1);
    }
    ...
    backupFiles.forEach((file) => {
        const group = file.originalName.includes('gmail')
            ? 'Gmail'
            : file.originalName.includes('openai')
            ? 'OpenAI'
            : file.originalName.includes('google')
            ? 'Google'
            : file.originalName.includes('aws')
            ? 'AWS'
            : file.originalName.includes('config')
            ? 'Configuration'
            : file.originalName.includes('state')
            ? 'State'
    ...
    if (options.force) {
        ...
        console.log(chalk.blue('\n🔄 Interactive Restore'));
        console.log(
            chalk.gray('Each file group will be prompted individually unless you use --force.')
        );
    }
    ...
    for (const [group, files] of Object.entries(fileGroups)) {
        ...
        if (shouldRestore) {
            ...
            console.log(
                chalk.green(`   📊 ${group}: ${groupRestored}/${files.length} files restored`)
            );
        } else {
