    ...
    try {
        ...
        if (!fs.existsSync(configPath)) {
            addError(
                'config.json',
                'File does not exist',
                'Create config.json in the project root with proper configuration'
            );
            return null;
        }
...
function checkGmailUsers(config) {
    ...
    if (!config.app) {
        addError(
            'Gmail Users',
            'config.json missing "app" section',
            'Add an "app" section to config.json with Gmail user configurations'
        );
        return false;
    }

    const requiredUsers = ['defaultPollGmailUser', 'defaultSendGmailUser', 'defaultGetGmailUser'];
    ...
    for (const userKey of requiredUsers) {
        ...
        if (!userEmail || userEmail.trim() === '') {
            addError(
                `Gmail Users - ${userKey}`,
                `Missing or empty: ${userKey}`,
                `Add a valid email address for ${userKey} in config.json app section`
            );
            ...
            addError(
                ...
                `Provide a valid email address for ${userKey} in config.json`
            );
            allUsersPresent = false;
        } else {
...
function checkOpenAI(config) {
    ...
    if (!config.openai) {
        addError(
            'OpenAI Config',
            'config.json missing "openai" section',
            'Add an "openai" section to config.json with llmModel configuration'
        );
        return false;
    }
    ...
    if (!config.openai.llmModel || config.openai.llmModel.trim() === '') {
        addError(
            'OpenAI Config',
            'Missing llmModel in config.json',
            'Add llmModel (e.g., "gpt-4o") to the openai section in config.json'
        );
        return false;
    }
    ...
    if (!apiKey || apiKey === '' || apiKey === 'your_api_key_here') {
        addError(
            'OpenAI .env',
            'OPENAI_API_KEY is empty or placeholder',
            'Replace "your_api_key_here" with your actual OpenAI API key in .env file'
        );
        return false;
    }
...
function checkAWS(config) {
    ...
    if (!config.aws) {
        addError(
            'AWS Config',
            'config.json missing "aws" section',
            'Add an "aws" section to config.json with AWS configuration'
        );
        return false;
    }
    ...
    if (!config.aws.region || config.aws.region.trim() === '') {
        addError(
            'AWS Config',
            'Missing AWS region in config.json',
            'Add region (e.g., "us-east-1") to the aws section in config.json'
        );
        return false;
    }
    ...
    if (!fs.existsSync(envPath)) {
        addError(
            'AWS .env',
            '.env file does not exist',
            'Create a .env file in the project root and add: AWS_PROFILE=your_profile_name'
        );
        return false;
    }
    ...
    if (!awsProfileMatch) {
        addError(
            'AWS .env',
            'AWS_PROFILE not found in .env file',
            'Add AWS_PROFILE=your_profile_name to the .env file'
        );
        return false;
    }
    ...
    if (!profile || profile === '' || profile === 'your_profile_name') {
        addError(
            'AWS .env',
            'AWS_PROFILE is empty or placeholder',
            'Replace "your_profile_name" with your actual AWS profile name in .env file'
        );
        return false;
    }
...
// Check email processing configuration
function checkEmailProcessing(config) {
    printSubHeader('Checking Email Processing Configuration');

    // Check if email processing is properly configured
    if (!config.email) {
        addWarning('Email Processing', 'config.json missing "email" section - will use defaults');
    } else {
        addPass('Email Processing', 'Email processing configuration section found');
    }

    // Check billing configuration
    if (!config.billing) {
        addWarning('Billing Config', 'config.json missing "billing" section - will use defaults');
    } else {
        addPass('Billing Config', 'Billing configuration section found');

        // Check specific billing parameters if present
        if (config.billing.freeTierLimit) {
            addPass('Billing Config', `Free tier limit: ${config.billing.freeTierLimit}`);
        }
        if (config.billing.infrastructureCost) {
            addPass('Billing Config', `Infrastructure cost: $${config.billing.infrastructureCost}`);
        }
        if (config.billing.licensingCost) {
            addPass('Billing Config', `Licensing cost: $${config.billing.licensingCost}`);
        }
    }
    ...
    if (!config.rateLimiting) {
        addWarning(
            'Rate Limiting',
            'config.json missing "rateLimiting" section - will use defaults'
        );
    } else {
        addPass('Rate Limiting', 'Rate limiting configuration section found');

        if (config.rateLimiting.emailRejectionRateLimit) {
            addPass(
                'Rate Limiting',
                `Email rejection rate limit: ${config.rateLimiting.emailRejectionRateLimit}/hour`
            );
        }
    }

    // Check if the new Lambda functions are expected to be deployed
    printInfo(
        'Email processor and response generator Lambda functions will be deployed via Terraform'
    );
...
function checkGoogleCredentials() {
    ...
    if (!fs.existsSync(credentialsPath)) {
        addError(
            'Google Credentials',
            'google_credentials.json does not exist',
            'Download Google OAuth credentials from Google Cloud Console and save as google_credentials.json in project root'
        );
        return false;
    }
    ...
    try {
        ...
        if (!credentials.installed && !credentials.web) {
            addError(
                ...
                'Ensure you downloaded OAuth 2.0 credentials (not service account) from Google Cloud Console'
            );
            return false;
        }
        ...
        if (credentials.installed) {
            if (!credentials.installed.client_id || !credentials.installed.client_secret) {
                addError(
                    'Google Credentials',
                    'Missing client_id or client_secret in credentials',
                    'Ensure your OAuth credentials contain both client_id and client_secret'
                );
                return false;
            }
        ...
        addError(
            'Google Credentials',
            `Invalid JSON in google_credentials.json: ${error.message}`,
            'Fix the JSON syntax in google_credentials.json'
        );
        return false;
    }
}
...
function checkGoogleTokens(config) {
    ...
    if (!fs.existsSync(tokensPath)) {
        addError(
            'Google Tokens',
            'data/google_tokens.json does not exist',
            'Run: npm run mail:authorize to authenticate Gmail users and generate tokens'
        );
        return false;
    }
    ...
    try {
        ...
        if (!tokens || typeof tokens !== 'object') {
            addError(
                'Google Tokens',
                'Invalid tokens format - not an object',
                'Run: npm run mail:authorize to regenerate valid tokens'
            );
            return false;
        }
        ...
        for (const userEmail of requiredUsers) {
            if (!tokens[userEmail]) {
                addError(
                    'Google Tokens',
                    `Missing tokens for user: ${userEmail}`,
                    `Run: npm run mail:authorize to authenticate user ${userEmail}`
                );
                ...
                if (!userTokens.access_token || !userTokens.refresh_token) {
                    addError(
                        'Google Tokens',
                        `Incomplete tokens for user: ${userEmail}`,
                        `Run: npm run mail:authorize to re-authenticate user ${userEmail}`
                    );
                    allUsersHaveTokens = false;
                } else {
        ...
        addError(
            'Google Tokens',
            `Invalid JSON in google_tokens.json: ${error.message}`,
            'Run: npm run mail:authorize to regenerate valid tokens'
        );
        return false;
    }
...
function main() {
    ...
    // Run all checks
    const gmailUsersResult = checkGmailUsers(config);
    const openaiResult = checkOpenAI(config);
    const awsResult = checkAWS(config);
    const emailProcessingResult = checkEmailProcessing(config);
    const credentialsResult = checkGoogleCredentials();
    const tokensResult = checkGoogleTokens(config);

    // Summary
    // eslint-disable-next-line no-unused-vars
    const gmailUsersOk = gmailUsersResult.success;
    // eslint-disable-next-line no-unused-vars
    const openaiOk = openaiResult.success;
    // eslint-disable-next-line no-unused-vars
    const awsOk = awsResult.success;
    // eslint-disable-next-line no-unused-vars
    const emailProcessingOk = emailProcessingResult.success;
    // eslint-disable-next-line no-unused-vars
    const credentialsOk = credentialsResult.success;
    // eslint-disable-next-line no-unused-vars
    const tokensOk = tokensResult.success;
    ...
    if (checkResults.failed > 0) {
        console.log(
            `${colors.red}❌ Prerequisites check failed. Please fix the errors above before proceeding.${colors.reset}\n`
        );

        console.log('Common solutions:');
        console.log(
            '1. For missing Gmail users: Update config.json app section with valid email addresses'
        );
        ...
        console.log(
            '4. For missing Google credentials: Download OAuth credentials from Google Cloud Console'
        );
        ...
        console.log(
            `${colors.green}✅ All prerequisites are satisfied! You can now run: npm run aws:init:prepare${colors.reset}\n`
        );
