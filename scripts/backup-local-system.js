...
function writeGlobalLog(action, backupDir, metadata) {
    try {
        ...
        const logLine = `[${timestamp}] ${action.toUpperCase()} - ${logEntry.name} - ${
            logEntry.notes
        } - ${logEntry.filesBackedUp} files - ${logEntry.backupDir}\n`;
...
function showUsage() {
    ...
    console.log(
        chalk.gray('  npm run backup:local --force            # Backup all without confirmation')
    );
    console.log(
        chalk.gray('  npm run backup:local --backup-dir ./my-backups # Custom backup location')
    );
    console.log(
        chalk.gray('  npm run backup:local --name "Pre-deployment" --notes "Before AWS migration"')
    );
    console.log('');
...
async function main() {
    ...
    if (options.force) {
        ...
        console.log(
            chalk.gray('This will backup sensitive configuration data from your local system.')
        );
    ...
    const logPath = writeBackupLog(
        options.backupDir,
        `BACKUP - ${backupName} - ${backupNotes || 'No notes'}`
    );
    ...
    if (totalBackedUp > 0) {
        ...
        console.log(
            chalk.yellow(
                `   • Test restore functionality: npm run restore:local ${options.backupDir}`
            )
        );
        console.log(
            chalk.yellow(`   • Consider encrypting backup directory for additional security`)
        );
    } else {
