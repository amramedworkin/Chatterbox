...
function showGlobalWarning() {
    ...
    console.log(
        chalk.red('This will clean ALL sensitive configuration data from your local system.')
    );
...
async function promptUser(question, defaultValue = false, allowEmpty = false) {
    ...
    const answer = await new Promise((resolve) => {
        rl.question(
            chalk.yellow(`${question}${defaultValue ? ` (${defaultValue})` : ''}: `),
            (ans) => {
                rl.close();
                resolve(ans.trim());
            }
...
function writeGlobalLog(action, backupDir, metadata) {
    try {
        ...
        const logLine = `[${timestamp}] ${action.toUpperCase()} - ${logEntry.name} - ${
            logEntry.notes
        } - ${logEntry.filesBackedUp} backed up, ${logEntry.filesCleaned} cleaned - ${
            logEntry.backupDir
        }\n`;
            ...
            if (content) {
                ...
                if (extracted) {
                    if (typeof extracted === 'object') {
                        ...
                        const displayValue = item.sensitive
                            ? `${extracted.substring(0, 8)}...${extracted.substring(
                                  extracted.length - 4
                              )}`
                            : extracted;
...
function showUsage() {
    ...
    console.log(
        chalk.gray('  --force        Skip individual confirmations (requires global confirmation)')
    );
    ...
    console.log(
        chalk.gray('  npm run clean:local                    # Interactive cleaning with backups')
    );
    console.log(
        chalk.gray(
    ...
    console.log(
        chalk.gray('  npm run clean:local --wipe             # Interactive cleaning, no backups')
    );
    console.log(
        chalk.gray('  npm run clean:local --force:quiet --wipe # Complete wipe, no confirmations')
    );
    console.log(
        chalk.gray('  npm run clean:local --backup-dir ./backups # Custom backup location')
    );
    console.log(
        chalk.gray('  npm run clean:local --name "Pre-migration" --notes "Before AWS migration"')
    );
    console.log('');
...
async function main() {
    ...
    if (!cleanName) {
        cleanName = await promptUser(
            '📝 Enter a name for this clean operation',
            'Local System Clean',
            true
        );
        if (!cleanName.trim()) {
    ...
    if (!cleanNotes) {
        cleanNotes = await promptUser(
            '📝 Enter notes for this clean operation (optional)',
            '',
            true
        );
    }
    ...
    // Interactive mode
        ...
        console.log(
            chalk.gray('This will clean sensitive configuration data from your local system.')
        );
    ...
    if (options.backupDir && !options.wipe) {
        ...
        logPath = writeCleanLog(
            options.backupDir,
            `CLEAN - ${cleanName} - ${cleanNotes || 'No notes'}`
        );
    }
    ...
    if (options.backupDir && !options.wipe) {
        ...
        console.log(
            chalk.gray(
                `   📝 To restore, copy files from backup directory back to their original locations`
            )
        );
