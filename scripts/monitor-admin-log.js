...
function formatLogEntry(entry) {
  ...
  return `${colors.bright}| ${entry.logId.toString().padStart(5, '0')}-${entry.sequence.toString().padStart(3, '0')} |${colors.reset} ${timestamp} | ${entry.user} | ${actionColor}${entry.actionType}${colors.reset} | ${entry.action} | ${entry.notes} |`;
}
...
function displayHeader() {
  console.log(`${colors.bright}${colors.cyan}=== Chatterbox Admin Log Monitor ===${colors.reset}`);
