...
const migrationItems = [
    ...
    },
    {
        ...
        type: 'pattern',
        instructions:
            'These files configure email sending tests:\n1. sendtest_sender_email.txt - sender email address\n2. sendtest_recipient_email.txt - recipient email address\n3. sendtest_send_count.txt - number of test emails sent\n4. Optional - used only for testing email sending functionality',
    },
    {
        name: 'Email Processing Configuration',
        description: 'Configuration for email processing and response generation',
        defaultPath: './config.json',
        required: false,
        type: 'config_section',
        instructions:
            'Email processing configuration:\n1. Review config.json for email processing settings\n2. Ensure OpenAI API key is configured in .env\n3. Verify Gmail user configuration\n4. Check billing and rate limiting settings\n5. Optional - will use defaults if not configured',
    },
    {
        name: 'Billing Configuration',
        description: 'Billing and cost management settings',
        defaultPath: './config.json',
        required: false,
        type: 'config_section',
        instructions:
            'Billing configuration:\n1. Review config.json for billing settings\n2. Set free tier limits and cost parameters\n3. Configure rate limiting for email processing\n4. Optional - will use default values if not configured',
    },
];
...
async function handleConfigSection(item, timestampPath) {
    printInfo(`Checking configuration for: ${item.name}`);

    // Check if config.json exists and has the required sections
    const configPath = path.join(process.cwd(), 'config.json');
    if (!fs.existsSync(configPath)) {
        printWarning(`config.json not found. ${item.name} will use default values.`);
        return { success: true, reason: 'Using default values' };
    }
    ...
    try {
        ...
        // Copy the config file to the timestamp folder
        const destPath = path.join(timestampPath, 'config.json');
        const result = copyFile(configPath, destPath);

        if (result.success) {
            printStatus(`Configuration file copied for ${item.name}`);
            return { success: true };
        } else {
            return result;
        }
        ...
        printWarning(
            `Error reading config.json: ${error.message}. ${item.name} will use default values.`
        );
        return { success: true, reason: 'Using default values due to config error' };
    }
}
...
async function handleMigrationItem(item, timestampPath) {
    ...
    if (hasData.toLowerCase() === 'n' || hasData.toLowerCase() === 'no') {
        if (item.required) {
            ...
            printWarning(
                `⚠️  ${item.name} is required but not ready. You'll need to provide it later.`
            );
    ...
    // Handle config_section type
    if (item.type === 'config_section') {
        return await handleConfigSection(item, timestampPath);
    }

    let sourcePath;
    if (item.type === 'env_file') {
        sourcePath = await question(
            `Enter the path to ${item.name} (or press Enter for default): `
        );
        if (!sourcePath.trim()) {
...
async function main() {
    ...
    try {
        ...
        if (!prereqResult.success) {
            printError(`Prerequisites check failed: ${prereqResult.errors.length} errors found`);
            console.log(
                '\nPlease fix the prerequisite issues above before proceeding with migration preparation.'
            );
            console.log(
                'You can run the prerequisite check separately with: node scripts/check-prerequisites.js\n'
            );
            rl.close();
            return { success: false, errors: prereqResult.errors };
        }
    } catch (error) {
    ...
    if (proceed.toLowerCase() !== 'y' && proceed.toLowerCase() !== 'yes') {
        ...
        rl.close();
        return { success: false, errors: ['User cancelled migration preparation'] };
    }
