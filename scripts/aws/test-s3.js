...
try {
  ...
  const dataBucket = execSync('cd Cloud/AWS/terraform && terraform output -raw s3_bucket_name', { encoding: 'utf8' }).trim();
  const backupBucket = execSync('cd Cloud/AWS/terraform && terraform output -raw s3_backup_bucket_name', { encoding: 'utf8' }).trim();
  ...
  for (const bucketInfo of buckets) {
    ...
    const bucketLocation = execSync(`aws s3api get-bucket-location --bucket ${bucketInfo.bucket} --profile cliadmin`, { encoding: 'utf8' });
    const location = JSON.parse(bucketLocation);
    console.log(chalk.green(`✅ Bucket location: ${location.LocationConstraint || 'us-east-1'}`));

    // Get bucket versioning
    console.log(chalk.yellow('Checking bucket versioning...'));
    const versioning = execSync(`aws s3api get-bucket-versioning --bucket ${bucketInfo.bucket} --profile cliadmin`, { encoding: 'utf8' });
    ...
    try {
      const encryption = execSync(`aws s3api get-bucket-encryption --bucket ${bucketInfo.bucket} --profile cliadmin`, { encoding: 'utf8' });
      const encryptionInfo = JSON.parse(encryption);
      console.log(chalk.green('✅ Server-side encryption configured:'));
      console.log(chalk.gray(`   Algorithm: ${encryptionInfo.ServerSideEncryptionConfiguration.Rules[0].ApplyServerSideEncryptionByDefault.SSEAlgorithm}`));
    } catch (error) {
    ...
    const publicAccess = execSync(`aws s3api get-public-access-block --bucket ${bucketInfo.bucket} --profile cliadmin`, { encoding: 'utf8' });
    ...
    try {
      const listResult = execSync(`aws s3 ls s3://${bucketInfo.bucket} --profile cliadmin`, { encoding: 'utf8' });
      if (listResult.trim()) {
    ...
    try {
      ...
      // Upload test file
      execSync(`aws s3 cp /tmp/${testFileName} s3://${bucketInfo.bucket}/test/ --profile cliadmin`, { stdio: 'inherit' });
      console.log(chalk.green('✅ Write access test successful'));
      
      // Download test file to verify
      execSync(`aws s3 cp s3://${bucketInfo.bucket}/test/${testFileName} /tmp/${testFileName}-download --profile cliadmin`, { stdio: 'inherit' });
      console.log(chalk.green('✅ Read access test successful'));
      
      // Clean up test files
      execSync(`aws s3 rm s3://${bucketInfo.bucket}/test/${testFileName} --profile cliadmin`, { stdio: 'inherit' });
