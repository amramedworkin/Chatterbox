...
async function confirmCleanup() {
  return new Promise((resolve) => {
    ...
    rl.question(chalk.red('\nAre you absolutely sure you want to proceed? Type "DESTROY" to confirm: '), (answer) => {
      rl.close();
...
async function main() {
  try {
    ...
    if (!currentDir.includes('Chatterbox')) {
      console.log(chalk.red('❌ Please run this script from the Chatterbox project root directory'));
      process.exit(1);
    }
    ...
    try {
      ...
      rl.question(chalk.yellow(`Do you want to delete the S3 backend bucket (${bucketName})? (y/N): `), (answer) => {
        if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
          try {
            // Empty the bucket first
            execSync(`aws s3 rm s3://${bucketName} --recursive --profile cliadmin`, { stdio: 'inherit' });
            // Delete the bucket
            execSync(`aws s3 rb s3://${bucketName} --profile cliadmin`, { stdio: 'inherit' });
            console.log(chalk.green('✅ S3 backend bucket deleted'));
          } catch (error) {
            console.log(chalk.yellow('⚠️  Could not delete S3 backend bucket'));
            console.log(chalk.gray('You may need to delete it manually from the AWS console'));
          }
        } else {
        ...
        console.log(chalk.gray('• If you want to remove the cliadmin user, run: npm run aws:cleanup:user'));
      });
