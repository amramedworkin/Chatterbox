...
async function updateSecret(secretType, options = {}) {
    ...
    if (!exists) {
        ...
        if (!dryRun) {
            try {
                execSync(
                    `aws secretsmanager create-secret --name "${secret.name}" --secret-string '${localSecretValue}' --description "${secret.description}" --profile cliadmin`,
                    { stdio: 'inherit' }
                );
        ...
        case 'same':
            if (force) {
                ...
                if (!dryRun) {
                    try {
                        execSync(
                            `aws secretsmanager put-secret-value --secret-id "${secret.name}" --secret-string '${localSecretValue}' --profile cliadmin`,
                            { stdio: 'inherit' }
                        );
