...
function checkAwsAdminUser() {
    try {
        ...
        return {
            success: false,
            error: 'AWS admin user not configured. Run: aws configure --profile cliadmin',
        };
    }
}
...
function checkGcpAccount() {
    return new Promise((resolve) => {
        rl.question(
            chalk.yellow('Do you have a GCP account with Gmail API enabled? (y/N): '),
            (answer) => {
...
function checkGmailCredentials() {
    return new Promise((resolve) => {
        rl.question(
            chalk.yellow(
                'Do you have Gmail API OAuth2 credentials (client_id, client_secret)? (y/N): '
            ),
            (answer) => {
                if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
                    resolve({ success: true });
...
function checkEmailAddresses() {
    return new Promise((resolve) => {
        rl.question(
            chalk.yellow('Do you have email addresses for polling, sending, and testing? (y/N): '),
            (answer) => {
                if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
                    resolve({ success: true });
                } else {
...
function checkOpenAIAccount() {
    return new Promise((resolve) => {
        rl.question(
            chalk.yellow('Do you have an OpenAI account with API access? (y/N): '),
            (answer) => {
                if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
                    resolve({ success: true });
                } else {
...
function checkExistingChatterSystem() {
    ...
    try {
        ...
        // eslint-disable-next-line no-unused-vars
        const groupExists = execSync(
            'aws iam get-group --group-name chatteradmingrp --profile cliadmin',
            { stdio: 'pipe' }
        );
        console.log(chalk.green('✅ chatteradmingrp group exists'));

        // Check if user is in group
        const userGroups = execSync(
            'aws iam list-groups-for-user --user-name chatteradmin --profile cliadmin',
            { encoding: 'utf8' }
        );
        ...
        if (inGroup) {
            ...
            console.log(
                chalk.yellow('⚠️  chatteradmin user exists but is not in chatteradmingrp group')
            );
