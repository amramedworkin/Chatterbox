...
try {
  ...
  const vpcId = execSync('cd Cloud/AWS/terraform && terraform output -raw vpc_id', { encoding: 'utf8' }).trim();
  console.log(chalk.green(`✅ VPC ID: ${vpcId}`));

  // Describe VPC
  console.log(chalk.yellow('Describing VPC...'));
  const vpcInfo = execSync(`aws ec2 describe-vpcs --vpc-ids ${vpcId} --profile cliadmin`, { encoding: 'utf8' });
  ...
  const subnetsInfo = execSync(`aws ec2 describe-subnets --filters Name=vpc-id,Values=${vpcId} --profile cliadmin`, { encoding: 'utf8' });
  const subnets = JSON.parse(subnetsInfo).Subnets;
  
  console.log(chalk.green(`✅ Found ${subnets.length} subnets:`));
  subnets.forEach((subnet, index) => {
    console.log(chalk.gray(`   ${index + 1}. ${subnet.SubnetId} (${subnet.CidrBlock}) - ${subnet.AvailabilityZone}`));
  });

  // Get route tables
  console.log(chalk.yellow('Getting route table information...'));
  const routeTablesInfo = execSync(`aws ec2 describe-route-tables --filters Name=vpc-id,Values=${vpcId} --profile cliadmin`, { encoding: 'utf8' });
  ...
  const securityGroupsInfo = execSync(`aws ec2 describe-security-groups --filters Name=vpc-id,Values=${vpcId} --profile cliadmin`, { encoding: 'utf8' });
  ...
  const vpcEndpointsInfo = execSync(`aws ec2 describe-vpc-endpoints --filters Name=vpc-id,Values=${vpcId} --profile cliadmin`, { encoding: 'utf8' });
  ...
  vpcEndpoints.forEach((endpoint, index) => {
    console.log(chalk.gray(`   ${index + 1}. ${endpoint.VpcEndpointId} (${endpoint.ServiceName})`));
  });
