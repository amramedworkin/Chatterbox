  ...
  return {
    ...
    Statement: [
      ...
      },
      {
        ...
        Resource: [
          "arn:aws:logs:us-east-1:*:log-group:/aws/chatterbox*"
        ]
      },
      {
        Effect: "Allow",
        ...
        Resource: [
          "arn:aws:iam::*:role/development-chatterbox-role"
        ]
      },
      {
        Effect: "Allow",
...
if (!userExists) {
  ...
  try {
    ...
    console.log('❌ Failed to create cliadmin user. You may need to use a different profile with admin permissions.');
    process.exit(1);
  }
} else {
...
const policyExists = runAwsCommand('aws iam get-policy --policy-arn arn:aws:iam::855581761117:policy/cliadmin-chatterbox-policy');
if (!policyExists) {
  ...
  try {
    execSync(`aws iam create-policy --policy-name cliadmin-chatterbox-policy --policy-document file://${policyFile}`, {
      stdio: 'inherit',
      env: { ...process.env, AWS_PROFILE: 'cliadmin' }
    });
    ...
    console.log('❌ Failed to create policy. You may need to use a different profile with admin permissions.');
...
if (attachedPolicies) {
  ...
  const hasPolicy = policies.AttachedPolicies.some(p => p.PolicyName === 'cliadmin-chatterbox-policy');
  
  if (!hasPolicy) {
    console.log('❌ Policy not attached to cliadmin user. Attaching...');
    try {
      execSync('aws iam attach-user-policy --user-name cliadmin --policy-arn arn:aws:iam::855581761117:policy/cliadmin-chatterbox-policy', {
        stdio: 'inherit',
        env: { ...process.env, AWS_PROFILE: 'cliadmin' }
      });
      console.log('✅ Policy attached successfully');
    } catch (error) {
      console.log('❌ Failed to attach policy. You may need to use a different profile with admin permissions.');
      process.exit(1);
    }
  } else {
