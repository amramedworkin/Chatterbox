...
try {
  ...
  const secretsList = execSync('aws secretsmanager list-secrets --profile cliadmin --filters Key=name,Values=chatterbox', { encoding: 'utf8' });
  ...
  for (const secret of secrets) {
    ...
    const isExpected = expectedSecrets.some(expected => secret.Name.includes(expected.replace('chatterbox/', '')));
    if (isExpected) {
    ...
    try {
      const secretMetadata = execSync(`aws secretsmanager describe-secret --secret-id ${secret.Name} --profile cliadmin`, { encoding: 'utf8' });
      const metadata = JSON.parse(secretMetadata);
      console.log(chalk.green(`     ✅ Access successful`));
      console.log(chalk.gray(`       Version Count: ${metadata.VersionIdsToStages ? Object.keys(metadata.VersionIdsToStages).length : 0}`));
    } catch (error) {
  ...
  const missingSecrets = expectedSecrets.filter(expected => 
  ...
  try {
    // Create test secret
    execSync(`aws secretsmanager create-secret --name ${testSecretName} --secret-string '${testSecretValue}' --profile cliadmin`, { stdio: 'inherit' });
    console.log(chalk.green('✅ Test secret created successfully'));

    // Verify secret exists
    const testSecretInfo = execSync(`aws secretsmanager describe-secret --secret-id ${testSecretName} --profile cliadmin`, { encoding: 'utf8' });
    const testSecret = JSON.parse(testSecretInfo);
    console.log(chalk.green(`✅ Test secret verified: ${testSecret.Name}`));

    // Delete test secret
    execSync(`aws secretsmanager delete-secret --secret-id ${testSecretName} --force-delete-without-recovery --profile cliadmin`, { stdio: 'inherit' });
