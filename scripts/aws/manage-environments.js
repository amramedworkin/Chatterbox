...
function getAccountId() {
  try {
    const result = execSync('aws sts get-caller-identity --profile cliadmin --query Account --output text', { 
      stdio: 'pipe',
      encoding: 'utf8'
    });
    return result.trim();
  } catch (error) {
    console.error(chalk.red('❌ Failed to get AWS account ID. Make sure AWS CLI is configured.'));
    process.exit(1);
  }
}
...
function setupBackend(environment) {
  ...
  try {
    ...
    execSync(`aws s3 mb s3://${bucketName} --profile cliadmin --region us-east-1`, { stdio: 'pipe' });
    console.log(chalk.green(`   ✅ S3 bucket: ${bucketName}`));
    
    // Enable versioning
    execSync(`aws s3api put-bucket-versioning --bucket ${bucketName} --versioning-configuration Status=Enabled --profile cliadmin`, { stdio: 'pipe' });
...
async function selectEnvironment() {
  ...
  switch (answer) {
    ...
    case '4': return Object.keys(AVAILABLE_ENVIRONMENTS);
    case '5': return [];
    default:
...
async function deployEnvironment(environment, options = {}) {
  ...
  try {
    ...
    execSync(`terraform plan -var-file="../environments/${environment}.tfvars" -out=${planFile}`, { stdio: 'inherit' });
    ...
    if (!force) {
      ...
      const answer = await new Promise((resolve) => {
        rl.question(chalk.yellow(`\n🤔 Do you want to deploy ${env.name} environment? (y/N): `), (ans) => {
          rl.close();
...
async function destroyEnvironment(environment, options = {}) {
  ...
  try {
    ...
    if (!force) {
      ...
      const answer = await new Promise((resolve) => {
        rl.question(chalk.red(`\n⚠️  Are you sure you want to destroy ${env.name} environment? This action cannot be undone! (yes/DESTROY): `), (ans) => {
          rl.close();
          resolve(ans.trim());
        });
      });
    ...
    execSync(`terraform destroy -var-file="../environments/${environment}.tfvars" -auto-approve`, { stdio: 'inherit' });
