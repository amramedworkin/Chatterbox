        ...
        const adminPolicy = {
            ...
            Statement: [
                // S3 Full Access
                {
                    Effect: 'Allow',
                    Action: ['s3:*'],
                    Resource: ['arn:aws:s3:::chatterbox-*', 'arn:aws:s3:::chatterbox-*/*'],
                },
                // IAM Management
                {
                    Effect: 'Allow',
                ...
                // DynamoDB Full Access
                {
                    Effect: 'Allow',
                    Action: ['dynamodb:*'],
                    Resource: ['arn:aws:dynamodb:us-east-1:*:table/chatterbox-*'],
                },
        ...
        for (const policyArn of managedPolicies) {
            try {
                ...
                if (error.name === 'EntityAlreadyExistsException') {
                    ...
                    console.log(
                        `⚠️  Could not attach ${policyArn.split('/').pop()}: ${error.message}`
                    );
                }
            }
        }
