    ...
    for (const [key, secret] of Object.entries(rotatableSecrets)) {
        ...
        if (metadata) {
            ...
            if (daysSinceModified > secret.rotationDays) {
                console.log(
                    chalk.red(
                        `   ⚠️  OVERDUE for rotation (${secret.rotationDays} days recommended)`
                    )
                );
            } else if (daysSinceModified > secret.rotationDays * 0.8) {
                console.log(
                    chalk.yellow(
                        `   ⚠️  Due for rotation soon (${
                            secret.rotationDays - daysSinceModified
                        } days remaining)`
                    )
                );
            } else {
...
async function rotateSecret(secretType) {
    ...
    if (metadata) {
        const lastModified = new Date(metadata.LastModifiedDate);
        console.log(
            chalk.gray(`Current secret was last modified: ${lastModified.toLocaleDateString()}`)
        );
    }
    ...
    const answer = await new Promise((resolve) => {
        rl.question(
            chalk.yellow('\nHave you completed the rotation steps above? (y/N): '),
            (ans) => {
...
async function main() {
    ...
    if (args.length === 0) {
        ...
        console.log(
            chalk.gray('  npm run aws:rotate:secrets                    # Interactive menu')
        );
        console.log(chalk.gray('  npm run aws:rotate:secrets status             # Show status'));
        console.log(
            chalk.gray('  npm run aws:rotate:secrets rotate <type>      # Rotate specific secret')
        );
        console.log('');
