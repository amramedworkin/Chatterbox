...
try {
  ...
  const tableName = execSync('cd Cloud/AWS/terraform && terraform output -raw dynamodb_table_name', { encoding: 'utf8' }).trim();
  console.log(chalk.green(`✅ Table Name: ${tableName}`));

  // Describe DynamoDB table
  console.log(chalk.yellow('Describing DynamoDB table...'));
  const tableInfo = execSync(`aws dynamodb describe-table --table-name ${tableName} --profile cliadmin`, { encoding: 'utf8' });
  ...
  if (table.GlobalSecondaryIndexes && table.GlobalSecondaryIndexes.length > 0) {
    console.log(chalk.green(`✅ Found ${table.GlobalSecondaryIndexes.length} global secondary indexes:`));
  ...
  try {
    const scanResult = execSync(`aws dynamodb scan --table-name ${tableName} --max-items 1 --profile cliadmin`, { encoding: 'utf8' });
  ...
  const metrics = execSync(`aws cloudwatch list-metrics --namespace AWS/DynamoDB --dimensions Name=TableName,Value=${tableName} --profile cliadmin`, { encoding: 'utf8' });
