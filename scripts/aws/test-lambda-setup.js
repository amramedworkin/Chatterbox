...
class LambdaSetupTester {
    ...
    async testTerraformOutputs() {
        ...
        try {
            ...
            const iamRoleArn = execSync('cd Cloud/AWS/terraform && terraform output -raw iam_role_arn', { encoding: 'utf8' }).trim();
            this.addResult('IAM Role ARN Output', true, `Role ARN: ${iamRoleArn}`);
            
            // Test Lambda function name output
            const lambdaFunctionName = execSync('cd Cloud/AWS/terraform && terraform output -raw lambda_function_name', { encoding: 'utf8' }).trim();
            this.addResult('Lambda Function Name Output', true, `Function Name: ${lambdaFunctionName}`);
            
            // Test API Gateway ID output
            const apiGatewayId = execSync('cd Cloud/AWS/terraform && terraform output -raw api_gateway_id', { encoding: 'utf8' }).trim();
    ...
    async testLambdaFunction() {
        ...
        try {
            ...
            this.addResult('Lambda Function Exists', true, `Function ARN: ${functionResponse.Configuration.FunctionArn}`);
            ...
            this.addResult('Lambda Runtime', runtimeCorrect, `Expected: ${TEST_CONFIG.lambda.expectedRuntime}, Got: ${config.Runtime}`);
            
            // Test memory
            const memoryCorrect = config.MemorySize === TEST_CONFIG.lambda.expectedMemory;
            this.addResult('Lambda Memory', memoryCorrect, `Expected: ${TEST_CONFIG.lambda.expectedMemory}MB, Got: ${config.MemorySize}MB`);
            
            // Test timeout
            const timeoutCorrect = config.Timeout === TEST_CONFIG.lambda.expectedTimeout;
            this.addResult('Lambda Timeout', timeoutCorrect, `Expected: ${TEST_CONFIG.lambda.expectedTimeout}s, Got: ${config.Timeout}s`);
            ...
            for (const varName of requiredVars) {
                const hasVar = envVars.hasOwnProperty(varName);
                this.addResult(`Environment Variable: ${varName}`, hasVar, hasVar ? `Value: ${envVars[varName]}` : 'Missing');
            }
    ...
    async testSecretsManager() {
        ...
        try {
            ...
            if (googleCreds.web) {
                hasClientId = googleCreds.web.hasOwnProperty('client_id');
                hasClientSecret = googleCreds.web.hasOwnProperty('client_secret');
                hasRedirectUris = googleCreds.web.hasOwnProperty('redirect_uris');
            }
            // Check for flat format (service account or legacy)
            else {
                hasClientId = googleCreds.hasOwnProperty('client_id');
                hasClientSecret = googleCreds.hasOwnProperty('client_secret');
                hasRedirectUris = googleCreds.hasOwnProperty('redirect_uris');
            }
            
            this.addResult('Google Credentials Secret Exists', true, 'Secret retrieved successfully');
            this.addResult('Google Credentials - Client ID', hasClientId, hasClientId ? 'Present' : 'Missing');
            this.addResult('Google Credentials - Client Secret', hasClientSecret, hasClientSecret ? 'Present' : 'Missing');
            this.addResult('Google Credentials - Redirect URIs', hasRedirectUris, hasRedirectUris ? 'Present' : 'Missing');
            ...
            try {
                ...
                this.addResult('Gmail Tokens - User Tokens Exist', hasTokens, hasTokens ? `Found ${userEmails.length} user(s): ${userEmails.join(', ')}` : 'No user tokens found');
                
                // Validate token format for first user (if exists)
                if (hasTokens) {
                    const firstUser = userEmails[0];
                    const userTokens = gmailTokens[firstUser];
                    const hasAccessToken = userTokens.hasOwnProperty('access_token');
                    const hasRefreshToken = userTokens.hasOwnProperty('refresh_token');
                    const hasScope = userTokens.hasOwnProperty('scope');
                    
                    this.addResult(`Gmail Tokens - ${firstUser} Access Token`, hasAccessToken, hasAccessToken ? 'Present' : 'Missing');
                    this.addResult(`Gmail Tokens - ${firstUser} Refresh Token`, hasRefreshToken, hasRefreshToken ? 'Present' : 'Missing');
                    this.addResult(`Gmail Tokens - ${firstUser} Scope`, hasScope, hasScope ? `Scope: ${userTokens.scope}` : 'Missing');
                }
            } catch (error) {
                if (error.name === 'ResourceNotFoundException') {
                    this.addResult('Gmail Tokens Secret Exists', false, 'Secret not found - this is expected for new setups');
                } else {
    ...
    async testS3Buckets() {
        ...
        try {
            ...
            this.addResult('S3 Email Archive Bucket Exists', true, `Bucket: ${TEST_CONFIG.s3.emailArchiveBucket}`);
    ...
    async testAPIGateway() {
        ...
        try {
            ...
            this.addResult('API Gateway Exists', true, `API Name: ${apiResponse.name}, ID: ${apiGatewayId}`);
            ...
            const lambdaResource = resourcesResponse.items.find(resource => 
                resource.resourceMethods && 
                resource.resourceMethods[TEST_CONFIG.apiGateway.expectedMethod]
            );
            
            this.addResult('API Gateway Lambda Resource', !!lambdaResource, 
                lambdaResource ? `Found resource with ${TEST_CONFIG.apiGateway.expectedMethod} method` : 'Lambda resource not found');
    ...
    async testIAMPermissions() {
        ...
        try {
            ...
            this.addResult('IAM Role Permissions', true, 'Role exists (permissions verified via Terraform)');
    ...
    async testEndToEnd() {
        ...
        try {
            ...
            try {
                ...
                if (response.ok) {
                    this.addResult('API Endpoint Accessible', true, 'Endpoint responds successfully');
                } else {
                    this.addResult('API Endpoint Accessible', true, `Endpoint responds (status: ${response.status})`);
                }
            } catch (error) {
                // This might fail if no emails exist, which is expected
                this.addResult('API Endpoint Test', true, `Endpoint test completed (may fail if no emails exist): ${error.message}`, true);
            }
