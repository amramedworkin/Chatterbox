...
try {
    ...
    const logStreamsOutput = execSync(
        `aws logs describe-log-streams --log-group-name "${LOG_GROUP_NAME}" --order-by LastEventTime --descending --max-items 1 --output json`,
        { encoding: 'utf8' }
    );
    ...
    const logEventsOutput = execSync(
        `aws logs get-log-events --log-group-name "${LOG_GROUP_NAME}" --log-stream-name "${latestStream.logStreamName}" --output json`,
        { encoding: 'utf8' }
    );
    ...
    try {
        const filterOutput = execSync(
            `aws logs filter-log-events --log-group-name "${LOG_GROUP_NAME}" --query 'events[*].message' --output text`,
            { encoding: 'utf8' }
        );
