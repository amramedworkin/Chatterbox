...
try {
  ...
  const accountId = execSync('aws sts get-caller-identity --profile cliadmin --query Account --output text', { encoding: 'utf8' }).trim();
  ...
  // Create S3 bucket
  try {
    execSync(`aws s3 mb s3://${bucketName} --profile cliadmin --region us-east-1`, { stdio: 'inherit' });
  ...
  execSync(`aws s3api put-bucket-versioning --bucket ${bucketName} --versioning-configuration Status=Enabled --profile cliadmin`, { stdio: 'inherit' });
  ...
  execSync(`aws s3api put-bucket-encryption --bucket ${bucketName} --server-side-encryption-configuration file:///tmp/encryption.json --profile cliadmin`, { stdio: 'inherit' });
  ...
  execSync(`aws s3api put-public-access-block --bucket ${bucketName} --public-access-block-configuration BlockPublicAcls=true,IgnorePublicAcls=true,BlockPublicPolicy=true,RestrictPublicBuckets=true --profile cliadmin`, { stdio: 'inherit' });
  ...
  if (fs.existsSync(mainTfPath)) {
    ...
    if (backendRegex.test(mainTfContent)) {
      mainTfContent = mainTfContent.replace(backendRegex, `bucket = "chatterbox-terraform-state-${accountId}"`);
    } else {
    ...
    profile = "cliadmin"
      ...
      if (terraformBlockRegex.test(mainTfContent)) {
        mainTfContent = mainTfContent.replace(terraformBlockRegex, `terraform {$1${backendConfig}\n}`);
      }
    }
