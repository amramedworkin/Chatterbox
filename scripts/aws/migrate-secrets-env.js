  ...
  for (const secret of SECRETS_TO_MIGRATE) {
    ...
    try {
      ...
      if (!exists) {
        ...
        if (!dryRun) {
          await createOrUpdateSecret(secretName, localSecretValue, `${secret.description} for ${env.name} environment`);
          createdCount++;
        }
        continue;
      }
      ...
      switch (comparison) {
        ...
        case 'different':
          ...
          if (!dryRun) {
            await createOrUpdateSecret(secretName, localSecretValue, `${secret.description} for ${env.name} environment`, force);
            updatedCount++;
          }
          break;
...
// Run migration
migrateSecretsForEnvironment(environment, options).then((success) => {
  if (success) {
