...
async function confirmCleanup() {
    return new Promise((resolve) => {
        ...
        console.log(
            chalk.red('This action cannot be undone and will result in complete data loss.')
        );
        ...
        rl.question(
            chalk.red(
                '\nAre you absolutely sure you want to proceed? Type "DESTROY ALL" to confirm: '
            ),
            (answer) => {
...
function checkCurrentUser() {
    try {
        const identity = execSync('aws sts get-caller-identity --profile cliadmin', {
            encoding: 'utf8',
        });
...
function checkChatterAdminUser() {
    try {
        ...
        const credentials = execSync(
            'aws configure export-credentials --profile cliadmin --format env',
            { encoding: 'utf8' }
        );
...
function destroyInfrastructureWithChatterAdmin(credentials) {
    ...
    try {
        ...
        try {
            ...
            console.log(
                chalk.yellow(
                    '⚠️  No Terraform state found - infrastructure may already be destroyed'
                )
            );
            return true;
        }
    ...
    try {
        ...
        try {
            ...
            try {
                // Remove all versions
                execSync(
                    `aws s3api list-object-versions --bucket ${bucketName} --query '{Objects: Versions[].{Key:Key,VersionId:VersionId}}' --output json | jq -r '.Objects[] | "(.Key) (.VersionId)"' | while read key version; do aws s3api delete-object --bucket ${bucketName} --key "$key" --version-id "$version"; done`,
                    { stdio: 'pipe' }
                );
            } catch (error) {
            ...
            try {
                execSync(
                    `aws s3api list-object-versions --bucket ${bucketName} --query '{Objects: DeleteMarkers[].{Key:Key,VersionId:VersionId}}' --output json | jq -r '.Objects[] | "(.Key) (.VersionId)"' | while read key version; do aws s3api delete-object --bucket ${bucketName} --key "$key" --version-id "$version"; done`,
                    { stdio: 'pipe' }
                );
            } catch (error) {
...
function cleanupIAMResourcesWithCliAdmin() {
    ...
    try {
        ...
        try {
            execSync(
                'aws iam remove-user-from-group --user-name chatteradmin --group-name chatteradmingrp --profile cliadmin',
                { stdio: 'pipe' }
            );
        ...
        try {
            const attachedPolicies = execSync(
                'aws iam list-attached-user-policies --user-name chatteradmin --profile cliadmin',
                { encoding: 'utf8' }
            );
            ...
            for (const policy of policies.AttachedPolicies) {
                if (
                    ...
                    policy.PolicyName.includes('cliadmin')
                ) {
                    console.log(chalk.yellow(`   Detaching ${policy.PolicyName}...`));
                    execSync(
                        `aws iam detach-user-policy --user-name chatteradmin --policy-arn ${policy.PolicyArn} --profile cliadmin`,
                        { stdio: 'pipe' }
                    );
                }
            }
        ...
        try {
            execSync('aws iam delete-user --user-name chatteradmin --profile cliadmin', {
                stdio: 'pipe',
            });
        ...
        try {
            execSync('aws iam delete-group --group-name chatteradmingrp --profile cliadmin', {
                stdio: 'pipe',
            });
        ...
        try {
            const policies = execSync('aws iam list-policies --scope Local --profile cliadmin', {
                encoding: 'utf8',
            });
            ...
            for (const policy of policyList.Policies) {
                if (
                    ...
                    execSync(
                        `aws iam delete-policy --policy-arn ${policy.PolicyArn} --profile cliadmin`,
                        { stdio: 'pipe' }
                    );
                }
            }
...
async function main() {
    try {
        ...
        if (chatterAdminCheck.success) {
            const infrastructureDestroyed = destroyInfrastructureWithChatterAdmin(
                chatterAdminCheck.credentials
            );
