...
async function testAdminPermissions(username = 'chatteradmin') {
    try {
        ...
        if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
            console.log(
                '⚠️  AWS credentials not found in environment variables. Please ensure AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY are set.'
            );
        ...
        try {
            ...
            if (chatterboxBuckets.length > 0) {
                console.log(
                    `   Chatterbox buckets: ${chatterboxBuckets.map((b) => b.Name).join(', ')}`
                );
        ...
        try {
            ...
            console.log(
                `✅ Secrets Manager access: Found ${secretsResponse.SecretList.length} secrets`
            );
            if (chatterboxSecrets.length > 0) {
                console.log(
                    `   Chatterbox secrets: ${chatterboxSecrets.map((s) => s.Name).join(', ')}`
                );
        ...
        try {
            ...
            console.log(
                `✅ Parameter Store access: Found ${paramsResponse.Parameters.length} parameters`
            );
            if (paramsResponse.Parameters.length > 0) {
                console.log(
                    `   Parameters: ${paramsResponse.Parameters.map((p) => p.Name).join(', ')}`
                );
                ...
                tests.push({
                    ...
                    status: 'WARN',
                    error: 'No chatterbox parameters',
                });
            }
        } catch (error) {
        ...
        try {
            ...
            if (logsResponse.logGroups.length > 0) {
                console.log(
                    `   Log groups: ${logsResponse.logGroups
                ...
                tests.push({
                    ...
                    status: 'WARN',
                    error: 'No chatterbox log groups',
                });
            }
        } catch (error) {
