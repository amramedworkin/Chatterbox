...
const {
    S3Client,
    GetObjectCommand, // eslint-disable-line no-unused-vars
...
async function dumpSecretsManager() {
    ...
    try {
        ...
        const chatterboxSecrets =
            listResponse.SecretList?.filter(
                (secret) =>
        ...
        for (const secret of chatterboxSecrets) {
            ...
            console.log(
                `  Version Count: ${
                    secret.SecretVersionsToStages
                        ? Object.keys(secret.SecretVersionsToStages).length
                        : 'N/A'
                }`
            );
            ...
            try {
                ...
                if (secretResponse.SecretString) {
                    if (secret.Name.includes('openai-api-key')) {
                        // OpenAI API key is a plain string
                        console.log(`  Value: ${secretResponse.SecretString.substring(0, 8)}... (hidden)`);
                    } else {
                        ...
                        const safeKeys = Object.keys(secretData).filter(
                            (key) =>
...
async function dumpS3Buckets() {
    ...
    // Add email processing buckets with pattern matching
    try {
        const { ListBucketsCommand } = require('@aws-sdk/client-s3');
        const bucketsResponse = await s3Client.send(new ListBucketsCommand());
        
        // Find email content buckets
        const contentBuckets = bucketsResponse.Buckets?.filter(bucket => 
            bucket.Name?.startsWith('chatterbox-email-content-development-')
        ) || [];
        
        // Find attachment buckets
        const attachmentBuckets = bucketsResponse.Buckets?.filter(bucket => 
            bucket.Name?.startsWith('chatterbox-attachments-development-')
        ) || [];
        
        // Add found buckets to the list
        contentBuckets.forEach(bucket => chatterboxBuckets.push(bucket.Name));
        attachmentBuckets.forEach(bucket => chatterboxBuckets.push(bucket.Name));
    } catch (error) {
        printWarning(`Could not list buckets for pattern matching: ${error.message}`);
    }
...
async function dumpDynamoDBTables() {
    ...
    const chatterboxTables = [`${ENVIRONMENT}-chatterbox-state-table`, 'chatterbox-state-table'];
    ...
    for (const tableName of chatterboxTables) {
        ...
        try {
            ...
            console.log(
                `  Size: ${
                    table.TableSizeBytes ? `${(table.TableSizeBytes / 1024).toFixed(1)} KB` : 'N/A'
                }`
            );
            ...
            if (scanResponse.ScannedCount > scanResponse.Items.length) {
                printWarning(
                    `    ... and ${
                        scanResponse.ScannedCount - scanResponse.Items.length
                    } more items`
                );
            }
        } catch (error) {
...
async function dumpLambdaFunctions() {
    ...
    try {
        ...
        const chatterboxFunctions =
            response.Functions?.filter(
                (func) =>
                    func.FunctionName?.includes('chatterbox') ||
                    func.FunctionName?.includes(ENVIRONMENT) ||
                    func.FunctionName?.includes('email-processor') ||
                    func.FunctionName?.includes('response-generator')
            ) || [];
            ...
            try {
                ...
                if (configResponse.Environment?.Variables) {
                    ...
                    for (const [key, value] of Object.entries(
                        configResponse.Environment.Variables
                    )) {
...
async function collectDataForJson() {
    ...
    try {
        ...
        for (const secret of chatterboxSecrets) {
            const secretData = {
                ...
                versionCount: secret.SecretVersionsToStages
                    ? Object.keys(secret.SecretVersionsToStages).length
                    : null,
            };
            ...
            try {
                ...
                if (secretResponse.SecretString) {
                    ...
                    secretData.safeKeys = Object.keys(secretContent).filter(
                        (key) =>
                            !key.toLowerCase().includes('token') &&
                            !key.toLowerCase().includes('secret') &&
                            !key.toLowerCase().includes('password') &&
    ...
    const chatterboxBuckets = [
        `${ENVIRONMENT}-chatterbox-email-archive`,
        // Email processing buckets (with pattern matching for random suffixes)
        ...
        for (const lambda of data.lambdaFunctions) {
            ...
            const relatedParams = (data.parameterStore || []).filter((param) => {
                ...
                if (
                    ...
                    param.value.includes(lambda.functionName)
                )
                    return true;
                if (
                    param.name &&
                    typeof param.name === 'string' &&
                    param.name.includes(lambda.functionName)
                )
                    return true;
                if (lambda.environmentVariables) {
                    for (const val of Object.values(lambda.environmentVariables)) {
                        if (
                            typeof val === 'string' &&
                            (param.value?.includes(val) || param.name?.includes(val))
                        )
                            return true;
                    }
                }
            ...
            let relatedS3 = {};
            if (lambda.environmentVariables) {
                for (const [, val] of Object.entries(lambda.environmentVariables)) {
...
`);
    ...
    const outputFile = process.argv.includes('--output')
        ...
        : isJson
        ? 'data/dump/aws-config.json'
        : 'data/dump/aws-config.txt';
