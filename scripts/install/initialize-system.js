...
function showGcpInstructions() {
    ...
    console.log(chalk.white('• Keep your credentials secure and never commit them to version control'));
    console.log(chalk.white('• The OAuth consent screen may need to be published for production use'));
    console.log(chalk.white('• You may need to verify your app if you plan to use it with many users'));
}
...
function showOpenAiInstructions() {
    ...
    console.log(chalk.yellow('\n⚠️  Important Notes:'));
    console.log(chalk.white('• API keys are sensitive - never share them or commit to version control'));
...
function showAwsInstructions() {
    ...
    console.log(chalk.yellow('\n⚠️  Important Notes:'));
    console.log(chalk.white('• Keep your access keys secure and never commit them to version control'));
...
async function promptForEmailAddresses() {
    ...
    const emails = await inquirer.default.prompt([
        ...
        },
        {
            ...
            default: function(answers) { return answers.pollEmail; },
            validate: (input) => {
                if (!input.trim()) return 'Email is required';
                if (!input.includes('@')) return 'Invalid email format';
                return true;
...
function createTestData() {
    ...
    const testFiles = [
        ...
        { name: 'sample.json', content: JSON.stringify({ test: true, message: 'Sample JSON data' }, null, 2) }
    ];
...
async function initializeSystem() {
    ...
    try {
        ...
        if (!fs.existsSync(CONFIG_PATH)) {
            console.error(chalk.red('❌ Not a valid Chatterbox installation. Please run this from a Chatterbox directory.'));
            process.exit(1);
        }
